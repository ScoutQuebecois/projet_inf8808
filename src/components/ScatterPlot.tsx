...
const CountryAvgScatter: React.FC<Props> = ({ data, selectedSport, selectedCountry, nocRegions}) => {

  const svgRef = useRef<SVGSVGElement>(null);

  useEffect(() => {

    if (!data.length || !svgRef.current || !selectedSport) return;
    ...
    const countryAverages = d3.rollups(
      filtered,
      athletes => ({
    ...
    const x = d3.scaleLinear()
      .domain(d3.extent(countryAverages, d => d.avgHeight) as [number, number])
      .nice()
      .range([0, innerWidth]);

    const y = d3.scaleLinear()
      .domain(d3.extent(countryAverages, d => d.avgWeight) as [number, number])
      .nice()
      .range([innerHeight, 0]);

    const allCountries = Array.from(
      new Set(data.map(d => d.NOC))
    );
    ...
    g.selectAll("circle")
      ...
      .attr("r", 5)
      .attr("fill", d =>
        nocRegions[d.NOC] === selectedCountry
          ? "black"
          : color(d.NOC)
      )
      .attr("stroke", "black")
