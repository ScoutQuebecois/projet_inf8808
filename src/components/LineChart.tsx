...
const LineChart: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    const enriched = filtered.map((d) => ({
      ...d,
      sportGroup: selectedSports.length === 0 ? "Tous les sports" : d.Sport,
      medalStatus: d.Medal && d.Medal !== "NA" ? "Medalists" : "Non-medalists",
    }));
    ...
    rolled.forEach(([sport, medalStatus, year, avg]) => {
      let group = formattedData.find(
        (g) => g.sport === sport && g.medalStatus === medalStatus,
      );
    ...
    const x = d3
      .scaleLinear()
      .domain(d3.extent(filtered, (d) => Number(d.Year)) as [number, number])
      .range([0, width]);
    ...
    const color = d3
      .scaleOrdinal<string>()
      .domain(selectedSports.length === 0 ? ["Tous les sports"] : selectedSports)
    ...
    (selectedSports.length === 0 ? ["Tous les sports"] : selectedSports).forEach(
      (sport, i) => {
        legend
          .append("rect")
          .attr("x", 0)
    ...
    const medalOffset = selectedSports.length * 20 + 50;
    ...
    formattedData.forEach((series) => {
      const sportColor = color(series.sport);
      ...
      g.append("path")
        ...
        .attr(
          "stroke-dasharray",
          series.medalStatus === "Medalists" ? "0" : "6,4",
        )
        .attr("d", line);
    ...
    >
      {!hasData && (
        ...
        >
          ...
          >
            (Aucune donnée correspondant à vos critères de sélection n'est
            disponible.)
          </span>
        </div>
      )}
